...
function setChapters() {
    try {
        let list = document.getElementById("about").getElementsByClassName("chapters")[0];
        if (list !== null && list !== undefined) {
            if (window.location.href.includes("details-libro1")) _setChaptersList(chaptersBook1, list);
            else if (window.location.href.includes("details-libro2")) _setChaptersList(chaptersBook2, list);
...
function nextBook() {
    if (window.location.href.includes("category")) {
        let el1 = document.getElementsByClassName("btnList")[1];
        el1.addEventListener("click", function() {
            let arr = document.querySelectorAll(".elementCat");
            for(let i = 0; i < arr.length; i++)if (arr[i].classList.contains("selected")) {
                arr[i].classList.remove("selected");
                if (i + 1 <= arr.length - 1) arr[i + 1].classList.add("selected");
                break;
            }
            counterCarousel += -230;
            arr.forEach((el)=>{
                el.style.transform = "translateX(" + counterCarousel + "px) scale(var(--scale-carousel))";
            });
...
function prevBook() {
    if (window.location.href.includes("category")) {
        let el3 = document.getElementsByClassName("btnList")[0];
        el3.addEventListener("click", function() {
            let arr = document.querySelectorAll(".elementCat");
            for(let i = arr.length - 1; i >= 0; i--)if (arr[i].classList.contains("selected")) {
                arr[i].classList.remove("selected");
                if (i - 1 >= 0) arr[i - 1].classList.add("selected");
                break;
            }
            counterCarousel += 230;
            arr.forEach((el)=>{
...
function _setDisabledBtnCarousel() {
    ...
    if (arr[0].classList.contains("selected")) prev.classList.add("disabled");
    else if (arr[arr.length - 1].classList.contains("selected")) next.classList.add("disabled");
    else {
...
//# sourceMappingURL=links.816e7b21.js.map
